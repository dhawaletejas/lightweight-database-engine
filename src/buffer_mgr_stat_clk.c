...
printPoolContent (BM_BufferPool *const bm)
{
  ...
  for (i = 0; i < bm->numPages; i++)
      printf("%s[%i%s%i%s%i]", ((i == 0) ? "" : ",") , frameContent[i], (dirty[i] ? "x": " "), fixCount[i]," ", refBits[i]);
  printf("\n");
}
...
sprintPoolContent (BM_BufferPool *const bm)
{
  ...
  for (i = 0; i < bm->numPages; i++)
    pos += sprintf(message + pos, "%s[%i%s%i%s%i]", ((i == 0) ? "" : ",") , frameContent[i], (dirty[i] ? "x": " "), fixCount[i], " ",refBits[i]);
